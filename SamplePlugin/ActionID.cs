        ...
        public override string ToString()
        {
            switch (Type)
            {
                case ActionType.Spell:
                    {
                        var actionData = DalamudApi.DataManager?.GetExcelSheet<Lumina.Excel.GeneratedSheets.Action>()?.GetRow(ID);
                ...
                case ActionType.Item:
                    {
                        ...
                        var itemData = DalamudApi.DataManager?.GetExcelSheet<Lumina.Excel.GeneratedSheets.Item>()?.GetRow(ID % 1000000);
